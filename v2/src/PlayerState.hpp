...
public:
    ...
    void updateView(const float mouse_dx, const float mouse_dy); // rotate camera
    std::unique_ptr<Turn> updatePosition(const GameTime&, const GameMap&); // check collisions here, displace camera
